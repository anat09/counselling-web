...
const ContactForm = () => {
  ...
  const [ error, setError ] = useState<{ error: string | undefined, success: boolean } | null>(null);
  ...
  const postEmail = async (): Promise<{ error: string | undefined, success: boolean }> => {
    if (!token) return { error: 'Validate the reCAPTCHA challenge before submitting', success: false }
  ...
  return (
    <section className="w-full font-philosopher max-w-2xl px-6 py-14 mx-auto rounded-md">
      ...
      <div className="mt-6 ">
        ...
        { error && <ErrorMessage message={error.error as string}/> }
